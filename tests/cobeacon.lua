-- cobeacon.lua -- coroutine.lua with flushed section beacons for
-- on-hardware crash localization. Auto-derived from coroutine.lua.
-- Each "Bnn <section>" line is unbuffered+flushed, so the LAST
...
_B("tests for multiple yield/resume arguments")
...
_B("bank-0 C stack (a full lua_resume + setjmp buffer + luaV_execute per")
-- bank-0 C stack (a full lua_resume + setjmp buffer + luaV_execute per
_B("level), so the chain depth = number of primes found is hard-capped at")
...
_B("ldo.c) raises a clean C stack overflow instead of overrunning the")
-- ldo.c) raises a clean "C stack overflow" instead of overrunning the
_B("segment. gennum=8 yields 4 primes (2,3,5,7) with margin. The old")
-- segment. gennum=8 yields 4 primes (2,3,5,7) with margin. The old
_B("gennum=46 (14 primes) actually overran the segment and corrupted")
-- gennum=46 (14 primes) actually overran the segment and corrupted
_B("memory; it only appeared to pass under GoldenGate.")
...
_B("yielding across C boundaries")
...
_B("access to locals of collected corroutines")
...
_B("old bug: attempt to resume itself")
...
_B("other old bug when attempting to resume itself")
...
_B("attempt to resume 'normal' coroutine")
...
_B("infinite recursion of coroutines")
...
_B("access to locals of erroneous coroutines")
...
_B("overwrite previous position of local `a'")
...
_B("leaving a pending coroutine open")
...
_B("repeat tests with larger constants (to use 'K' opcodes)")
...
_B("testing coroutines with C bodies")
...
_B("testing chain of suspendable C calls")
