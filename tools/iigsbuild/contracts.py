...
Intentional skips stay visible in results. (Moved from tools/test_support.py,
...
def compact_manifest() -> dict:
    """Bytecode acceptance plan for the parser-free runtime (tests/compact.json)."""
    data = json.loads((ROOT / "tests/compact.json").read_text())
