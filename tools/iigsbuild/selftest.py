...
def main(args) -> int:
    ...
    legacy = ["src/lapi.a", "src/lua.root", "src/lua", "src/parseconf.h", "bridge", "test.a", "tests/iigshost"]
    loose = ["build/lua", "build/lua.lib"]
    tree.make("test", "CHECKS=unit")  # creates build/test-runs
    for name in evidence + legacy + loose:
    ...
    check(not any((tree.root / n).exists() for n in legacy), "legacy in-tree objects and outputs removed")
    check(all((tree.root / n).exists() for n in loose), "legacy loose outputs left for clean-legacy")
    check(all((tree.root / n).is_file() for n in ("src/lapi.c", "src/luaconf.h", "Makefile")), "tracked files untouched")
    tree.make("clean-legacy")
    check(not any((tree.root / n).exists() for n in loose) and
          len(list((tree.root / "build/archive").glob("legacy-*/lua"))) == 1, "clean-legacy archives, not deletes")
