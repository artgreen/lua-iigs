...
Check groups:
  ...
  compact   bytecode acceptance on parser-free Lua (tests/hardware/compact.json),
...
from .testfiles import stage_sources
...
LUAC_CASES = (("hwsmoke", ("lua",)), ("numconv", ("lua", "luasmall")),
...
def expected_checks(groups=ALL_CHECKS):
    """Coverage contract for reports, independent of whether a run finished."""
    rows = [("unit", "python unittest", "tools")]
    rows += [(g, name, exe) for g, exe in (("targeted", "lua"), ("trace", "luatrace")) for name in TESTS]
    rows += [("luac", f"{name} {form}", "luac+" + "+".join(runtimes))
             for form in ("debug", "stripped") for name, runtimes in LUAC_CASES]
    rows += [("luac", name, exe) for name, exe in (("syntax error rejected", "luac"),
             ("nesting rejected", "luac"), ("valid compile after errors", "luac+lua"))]
    plan = compact_manifest()
    for form in ("debug", "stripped"):
        rows.append(("compact", f"precompile {form}", "luac"))
        rows += [("compact", f"{name} {form}", "luasmall") for name in plan["groups"][form]]
    rows += [("compact", name, "luasmall") for name in
             ("source script rejected", "stdin source rejected", "works after rejections")]
    rows += [("hosts", name, exe) for name, exe in (("iigshost", "iigshost"), ("allocfail", "allocfail"),
             ("bridge demo", "bridge"), ("bridge cstack", "bridge"), ("mmtest", "mmtest"), ("memfree", "memfree"))]
    return [row for row in rows if row[0] in groups]
...
class Session:
    ...
    def luac(self):
        ...
        for strip, form in ((False, "debug"), (True, "stripped")):
            for test, runtimes in LUAC_CASES:
    ...
    def hosts(self):
        ...
        shutil.copyfile(ROOT / "examples/embedding/bridge.lua", stage / "bridge.lua")
    ...
    def unit(self):
        def fn():
            ...
            if self.tc.iix:
                env["IIX"] = self.tc.iix
            result = subprocess.run([sys.executable, "-m", "unittest", "discover", "-s", str(ROOT / "tests/tooling"),
            ...
            if result.returncode or ran is None or int(ran.group(1)) == 0:
...
def test_inputs() -> Dict[str, str]:
    files = sorted(p for base in (ROOT / "tests", ROOT / "tools", ROOT / "examples")
                   for p in base.rglob("*") if p.is_file() and p.suffix in (".lua", ".json", ".c", ".h", ".py"))
...
def run_tests(args) -> int:
    ...
    inputs = test_inputs()
    executable_hashes = {n: sha256_file(p) for n, p in subject["paths"].items()}
    ...
    if test_inputs() != inputs or {n: sha256_file(p) for n, p in subject["paths"].items()} != executable_hashes:
        raise BuildError("test inputs or executables changed during the run; rerun against stable inputs")
    from collections import Counter
    if Counter((r["group"], r["name"], r["subject"]) for r in session.results) != Counter(expected_checks(checks)):
        raise BuildError("test run did not produce the expected coverage; refusing a complete report")
    failed = [r for r in session.results if r["status"] == "FAILED"]
    built_only = sum(r["status"] == "built, not run" for r in session.results)
    report = {
    ...
    executed = len(session.results) - built_only
    say(f"{executed - len(failed)}/{executed} checks passed"
        f" ({report['summary']['with_skips']} with intentional skips; {built_only} built only). "
        f"Report: {rel(work / 'report.json')}")
