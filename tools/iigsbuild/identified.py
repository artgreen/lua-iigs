...
HOST_HEADERS = ("examples/embedding/testiface.h",)
...
def runtime_inputs(tree: Path = ROOT) -> Dict[str, bytes]:
    if (tree / "src/parseconf.h").exists():
        raise BuildError("src/parseconf.h exists: it is generated per build now. Remove that stray file manually "
...
def git_state(files: Dict[str, bytes]) -> dict:
    def git(*args):
        try:
            ...
            return ""
    # Source archives may be unpacked inside another checkout: never use its Git identity.
    top = git("rev-parse", "--show-toplevel")
    if not top or Path(top).resolve() != ROOT.resolve():
        source = read_json(ROOT / "SOURCE-MANIFEST.json", {})
        hashes = source.get("files_sha256", {})
        differs = sorted(name for name, data in files.items() if hashes.get(name) != sha256(data))
        return {"commit": source.get("commit", ""), "branch": "source archive",
                "inputs_match_commit": bool(source.get("commit")) and not differs,
                "inputs_differing_from_commit": differs}
    tree = {}
