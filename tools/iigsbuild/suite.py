"""Run the reusable Lua suite (tests/hardware/suite.json + tests/hardware/suite.lua) locally
...
def prepare(work: Path, exe: Path, data: dict) -> Path:
    ...
    shutil.copyfile(ROOT / "tests/hardware/suite.lua", stage / "test.lua")
    for name in [*data["tests"], "tracegc"]:
        shutil.copyfile(test_source(name + ".lua"), stage / (name + ".lua"))
    return stage
...
def run_suite(args) -> int:
    ...
    report = {"schema": "lua-iigs-suite-run/1", "utc": utc_iso(), "group": args.group, "subject": subject,
              ...
              "manifest_sha256": sha256((ROOT / "tests/hardware/suite.json").read_bytes()),
