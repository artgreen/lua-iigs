...
def executables(args, tc, needed: List[str]) -> dict:
    ...
    for name in needed:
        member = names[name]
        spec = manifest.get("executable_containers", {}).get(member, {"archive": member + ".SHK"})
        if "zip" in spec:
            import tempfile
            try:
                with zipfile.ZipFile(release / spec["zip"]) as bundle:
                    archive_bytes = bundle.read(spec["archive"])
            except (OSError, zipfile.BadZipFile, KeyError) as exc:
                raise BuildError(f"cannot read {member} container in release: {exc}") from exc
            with tempfile.TemporaryDirectory() as temp:
                archive = Path(temp) / "input.shk"
            ...
            data = subprocess.run([tc.nulib2, "-p", str(archive), member], capture_output=True).stdout
        if sha256(data) != manifest.get("executables_sha256", {}).get(member):
...
def suite_kit(args, tc, exe) -> dict:
    ...
    files = {"TEST.LUA": (ROOT / "tests/hardware/suite.lua").read_bytes(),
             "SUITECFG.LUA": ("return " + lua_value(data) + "\n").encode()}
    for name in [*data["tests"], "tracegc"]:
        files[name.upper() + ".LUA"] = test_source(name + ".lua").read_bytes()
...
def small_kit(args, tc, exe, work: Path) -> dict:
    ...
    binary = {}
    for name, text in (("test", (ROOT / "tests/hardware/suite.lua").read_bytes()),
    ...
    for name in chunks:
        text[name.upper() + ".LUA"] = test_source(name + ".lua").read_bytes()
    ...
    return {"volume": "LUACOMPACT", "title": "Compact Lua bytecode acceptance. Chunks compiled here by LUACTEST.",
            ...
            "readme": ["Compact (parser-free) Lua acceptance. LUACTEST (same build) compiles the test",
                       ...
                       f"stripped ({len(groups['stripped'])} chunks). Parser-dependent tests: tests/hardware/compact.json.",
                       "tableovf previously took about 18 minutes and is silent until it completes.",
...
def luac_kit(args, tc, exe) -> dict:
    ...
    return {"volume": "LUACTEST", "title": LUAC_TITLE,
            ...
            "text": {"TEST.LUA": (ROOT / "tests/hardware/luacprobe.lua").read_bytes()},
...
def host_kit(args, tc, exe) -> dict:
    ...
    return {"volume": "HOSTCHECK", "title": HOST_TITLE,
            ...
            "text": {"TEST.LUA": (ROOT / "tests/hardware/hostcheck.lua").read_bytes(),
                     "HWSMOKE.LUA": (ROOT / "tests/regression/hwsmoke.lua").read_bytes()},
            "steps": steps, "preflight_steps": steps, "groups": {},
