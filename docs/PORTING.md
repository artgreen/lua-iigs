# Porting Lua 5.4.6 to the Apple IIgs

The upstream source is Lua 5.4.6, archive SHA-256
`7d5ea1b9cb6aa0b59ca3dde1c6adcb57ef83a1ba8e5432c0ecd06bf439b3ad88`.
The runtime sources were preserved byte for byte from development commit
`614a65bd7f3b6fdee297b57ceafd16b80a5ba697` during this repository reorganization.
The original upstream comparison found 41 of 61 C/header files changed;
22 of those contain only compiler/segment directives. The remaining changes
adapt Lua to ORCA/C, 16-bit C integers, bank-0 stack limits, and the Memory Manager.

The build generates a separate `parseconf.h` for each configuration.
The build tooling also retains the fixes merged through commit
`41f5179fec31f4e7d2119df9ca30cf06e4b950e6`. The runtime and build recipe retain their established behavior. Directory
organization, test staging, user documentation, and distribution packaging
are maintained separately from the language implementation.
...
| Build files | Replaced the upstream `src/Makefile` with the root build interface; `parseconf.h` is generated per build rather than tracked; all upstream C sources and headers are retained. |
...
[building](BUILDING.md), [embedding](EMBEDDING.md), and the [validation status](VALIDATION.md) for those supporting
