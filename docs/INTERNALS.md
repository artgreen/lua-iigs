...
These protections are retained in the current Lua 5.4.6 port. See
[validation](VALIDATION.md) for observed coverage and [embedding](EMBEDDING.md)
for the host initialization contract.
...
The upstream T C API harness is unavailable; focused C-host checks do not
replace it. Additional hardware configurations remain untested. The known
GoldenGate EOF/text differences and hardware sharing limits are documented
in [compatibility](COMPATIBILITY.md). [Validation](VALIDATION.md) records the
scope of hardware evidence without treating emulator results as hardware runs.
