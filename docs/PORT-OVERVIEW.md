...
interface, diagnostic, or functional changes. The source Makefile is replaced
and `parseconf.h` is added. These are file counts, not a measure of risk or
...
| Build and static linking | Replaced the Unix build with `iix compile`, `link`, and `makelib`; added a generated `parseconf.h` distinguishing interpreter/compiler builds. Failed compiles/library builds discard partial outputs. The VM object is linked separately from `lua.lib`. | Produces ORCA shell executables and an embeddable static library using the IIgs toolchain. | [src/Makefile](../src/Makefile), [parseconf.h](../src/parseconf.h), [root Makefile](../Makefile) |
...
| Optional parser-free interpreter | Added conditional lexer/parser initialization and code, plus interpreter option/REPL changes controlled by `LUA_NO_PARSER` and build identity. | An experimental smaller, bytecode-oriented build. Disabled in the validated distribution; it is not a claim that the port cannot execute source scripts. | [lua.c](../src/lua.c), [ldo.c](../src/ldo.c), [llex.c](../src/llex.c), [lparser.c](../src/lparser.c), [lstate.c](../src/lstate.c), [parseconf.h](../src/parseconf.h) |
...
| Build files | Replaced `src/Makefile`; added `src/parseconf.h`; no upstream `src/` files removed. |
